...
# =========================
# OAUTH STATE STORE (server-side, additive)
# =========================
# Prevents "OAuth state mismatch" when sessions/cookies are lost across redirects
# (common behind proxies, multi-tab connect attempts, or strict cookie policies).
OAUTH_STATE_PATH = DATA / "oauth_states.json"

def _load_oauth_states() -> Dict[str, Any]:
    try:
        data = load_json(OAUTH_STATE_PATH, {"states": {}})
...
def _save_oauth_states(data: Dict[str, Any]) -> None:
    try:
        ...
        save_json(OAUTH_STATE_PATH, data)
    except Exception:
        pass

def _put_oauth_state(state: str, username: str, kind: str, ttl_seconds: int = 900) -> None:
    try:
        if not state:
            return
        ...
        data["states"][state] = {
            ...
            "kind": kind or "",
            "created_at": now,
            "expires_at": now + max(60, min(86400, int(ttl_seconds))),
        }
        _save_oauth_states(data)
    except Exception:
        pass

def _peek_oauth_state(state: str, kind: str) -> Optional[Dict[str, Any]]:
    try:
        data = _load_oauth_states()
        rec = (data.get("states") or {}).get(state)
        if not isinstance(rec, dict):
            return None
        if (rec.get("kind") or "") != (kind or ""):
            return None
        exp = int(rec.get("expires_at") or 0)
        if exp and _now_epoch() > exp:
            return None
        return rec
    except Exception:
        return None

def _consume_oauth_state(state: str, kind: str) -> Optional[Dict[str, Any]]:
    try:
        data = _load_oauth_states()
...
def gmail_connect():
    ...
    # Store in session for standard flow
    try:
        session["gmail_oauth_state"] = state
    except Exception:
        pass

    # Also store server-side to survive session/cookie loss during OAuth redirects
    try:
        _put_oauth_state(state, u.get("username", ""), kind="gmail", ttl_seconds=900)
    except Exception:
        pass
...
def gmail_callback():
    ...
    if not state:
        return make_response("Missing OAuth state from Google.", 400)

    # If the session was lost, recover the user from the server-side state store
    if not session.get("user"):
        rec = _peek_oauth_state(state, kind="gmail")
        if rec and rec.get("user"):
            session["user"] = rec.get("user")

    u = current_user()
    if not u:
        # As a last resort, consume state and restore user again
        rec2 = _consume_oauth_state(state, kind="gmail")
        if rec2 and rec2.get("user"):
            session["user"] = rec2.get("user")
            u = current_user()
        if not u:
            return redirect("/login")

    # Accept session state match OR server-side state match.
    expected = (session.get("gmail_oauth_state", "") or "").strip()
    session_match = bool(expected and expected == state)

    rec = None
    if not session_match:
        rec = _consume_oauth_state(state, kind="gmail")

    if (not session_match) and (not rec):
        return make_response("OAuth state mismatch. Please retry Gmail connect.", 400)

    # We now have a valid state. Consume it if we haven't already.
    if session_match:
        try:
            _consume_oauth_state(state, kind="gmail")
        except Exception:
            pass
...
def calendar_connect():
    ...
    try:
        ...
        pass
    try:
        _put_oauth_state(state, u.get("username", ""), kind="calendar", ttl_seconds=900)
    except Exception:
        pass
...
def calendar_callback():
    ...
    if not session.get("user"):
        rec = _peek_oauth_state(state, kind="calendar")
        if rec and rec.get("user"):
            session["user"] = rec.get("user")

    u = current_user()
    if not u:
        rec2 = _consume_oauth_state(state, kind="calendar")
        if rec2 and rec2.get("user"):
            session["user"] = rec2.get("user")
            u = current_user()
        if not u:
    ...
    expected = (session.get("calendar_oauth_state", "") or "").strip()
    session_match = bool(expected and expected == state)
    rec = None
    if not session_match:
        rec = _consume_oauth_state(state, kind="calendar")
    if (not session_match) and (not rec):
        return make_response("OAuth state mismatch. Please retry Google Calendar connect.", 400)

    if session_match:
        try:
            _consume_oauth_state(state, kind="calendar")
        except Exception:
            pass
