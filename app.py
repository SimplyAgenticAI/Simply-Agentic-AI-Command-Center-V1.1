...
# === Stability Build Additive Patch (single-file, pre-__main__) ===

def _masked_key_hint(val: str) -> str:
    val = (val or "").strip()
    if not val:
        return ""
    if len(val) <= 4:
...
def _user_settings_record(u: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    u = u or current_user() or {}
    s = (u.get("settings") or {})
    if not isinstance(s, dict):
        s = {}
    s.setdefault("provider", "openai")
    s.setdefault("openai_key", "")
    s.setdefault("openai_model", OPENAI_MODEL or MODEL)
    s.setdefault("claude_key", "")
    s.setdefault("claude_model", CLAUDE_MODEL or "claude-3-5-sonnet-latest")
    s.setdefault("smtp", {})
    return s

def _effective_openai_key(u: Optional[Dict[str, Any]] = None) -> str:
    s = _user_settings_record(u)
    return (s.get("openai_key") or OPENAI_API_KEY or "").strip()

def _effective_openai_model(u: Optional[Dict[str, Any]] = None) -> str:
    s = _user_settings_record(u)
    return (s.get("openai_model") or OPENAI_MODEL or MODEL or "gpt-5.2").strip()

def _effective_claude_key(u: Optional[Dict[str, Any]] = None) -> str:
    s = _user_settings_record(u)
    return (s.get("claude_key") or CLAUDE_API_KEY or "").strip()

def _effective_claude_model(u: Optional[Dict[str, Any]] = None) -> str:
    s = _user_settings_record(u)
    return (s.get("claude_model") or CLAUDE_MODEL or "claude-3-5-sonnet-latest").strip()

def _effective_text_provider(u: Optional[Dict[str, Any]] = None) -> str:
    s = _user_settings_record(u)
    p = (s.get("provider") or AI_PROVIDER or "openai").strip().lower()
    if p not in ("openai", "claude"):
        p = "openai"
    if p == "claude" and not _effective_claude_key(u):
        return "openai"
    return p
...
def _flatten_text_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    for m in messages or []:
        ...
        if isinstance(c, list):
            ...
            c2 = str(c or "")
        safe_msgs.append({"role": str(m.get("role") or "user"), "content": c2})
    return safe_msgs

def _openai_text_call(system: str, messages: List[Dict[str, Any]], temperature: float = 0.6, u: Optional[Dict[str, Any]] = None) -> str:
    key = _effective_openai_key(u)
    model = _effective_openai_model(u)
    ...
    try:
        ...
        safe_msgs = _flatten_text_messages(messages)
...
def _claude_text_call(system: str, messages: List[Dict[str, Any]], temperature: float = 0.6, u: Optional[Dict[str, Any]] = None) -> str:
    if anthropic is None:
        raise RuntimeError("anthropic package is not installed on the server")
    key = _effective_claude_key(u)
    if not key:
        raise RuntimeError("Claude API key is missing")
    model = _effective_claude_model(u)
    safe_msgs = _flatten_text_messages(messages)
    anth_msgs = []
    if system.strip():
        anth_msgs.append({"role": "user", "content": "System instructions:\n" + system.strip()})
        anth_msgs.append({"role": "assistant", "content": "Understood. I will follow those instructions."})
    anth_msgs.extend([m for m in safe_msgs if (m.get("role") or "user") in ("user", "assistant")])
...
def call_llm(system: str, messages: List[Dict[str, Any]], temperature: float = 0.6) -> str:
    ...
    provider = _effective_text_provider(u)
    if provider == "claude":
        try:
            return _claude_text_call(system, messages, temperature=temperature, u=u)
        except Exception as e:
            # graceful fallback to OpenAI if configured
            if _effective_openai_key(u):
                out = _openai_text_call(system, messages, temperature=temperature, u=u)
                note = str(e).strip()
                if note:
                    return out + "\n\n[Claude fallback note: " + note + "]"
                return out
            raise
    return _openai_text_call(system, messages, temperature=temperature, u=u)

def _system_startup_report() -> Dict[str, Any]:
    packages = {
        "openai": True,
        "anthropic": anthropic is not None,
        "google_oauth": bool(GoogleOAuthFlow),
        "google_api": bool(google_build),
    }
    storage_ok = False
    storage_reason = ""
    try:
        test_path = DATA / "_health_write_test.tmp"
        test_path.write_text("ok", encoding="utf-8")
        test_path.unlink(missing_ok=True)
        storage_ok = True
    except Exception as e:
        storage_ok = False
        storage_reason = str(e)
    return {
        "timestamp": now_iso(),
        "data_dir": str(DATA),
        ...
        "packages": packages,
        "oauth_env_ready": bool(GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET and PUBLIC_BASE_URL),
        "openai_env_key": bool((OPENAI_API_KEY or "").strip()),
        "claude_env_key": bool((CLAUDE_API_KEY or "").strip()),
    }

STARTUP_REPORT = _system_startup_report()

def _system_health_snapshot(run_live: bool = False) -> Dict[str, Any]:
    from flask import has_request_context
    u = current_user() if has_request_context() else None
    settings = _user_settings_record(u)
    provider = _effective_text_provider(u)
    openai_key = _effective_openai_key(u)
    claude_key = _effective_claude_key(u)
    speech_ready = bool(openai_key)
    image_ready = bool(openai_key)
    gmail_connected = False
    calendar_connected = False
    try:
        gmail_connected = bool(_user_gmail_oauth(u)) if u else False
    except Exception:
        gmail_connected = False
    try:
        tok, _reason = _calendar_creds_for_user(u) if u else (None, "")
        calendar_connected = bool(tok)
    except Exception:
        calendar_connected = False
    return {
        "ok": True,
        ...
        "provider": provider,
        "text_model": _effective_claude_model(u) if provider == "claude" else _effective_openai_model(u),
        "keys": {
            "openai": {"present": bool(openai_key), "hint": _masked_key_hint(openai_key)},
            "claude": {"present": bool(claude_key), "hint": _masked_key_hint(claude_key)},
        },
        "capabilities": {
            "text": bool((provider == "claude" and claude_key) or (provider == "openai" and openai_key)),
            "image": image_ready,
            "speech_to_text": speech_ready,
            "gmail": gmail_connected,
            "calendar": calendar_connected,
        },
        "packages": STARTUP_REPORT.get("packages", {}),
        "oauth_env_ready": STARTUP_REPORT.get("oauth_env_ready", False),
        "storage_ok": STARTUP_REPORT.get("storage_ok", False),
        "notes": {
            "image": "" if image_ready else "OpenAI key required for image generation",
            "speech_to_text": "" if speech_ready else "OpenAI key required for transcription fallback",
            "claude": "" if anthropic is not None else "anthropic package is not installed",
        }
    }

@app.get("/api/user/settings/v2")
def api_get_user_settings_v2():
    u = current_user()
    if not u:
        return jsonify({"ok": False, "error": "Not authenticated"}), 401
    settings = _user_settings_record(u)
    ...
    return jsonify({
        ...
        "settings": {
            "provider": _effective_text_provider(u),
            "has_openai_key": bool(_effective_openai_key(u)),
            "openai_key_hint": _masked_key_hint(_effective_openai_key(u)),
            "openai_model": _effective_openai_model(u),
            "has_claude_key": bool(_effective_claude_key(u)),
            "claude_key_hint": _masked_key_hint(_effective_claude_key(u)),
            "claude_model": _effective_claude_model(u),
            "gmail_oauth_connected": bool((settings.get("gmail_oauth") or {})),
            "smtp": safe_smtp
        },
        "health": _system_health_snapshot(run_live=False)
    })

@app.post("/api/user/settings/v2")
def api_set_user_settings_v2():
    u = current_user()
    if not u:
        return jsonify({"ok": False, "error": "Not authenticated"}), 401
    ...
    openai_model = (data.get("openai_model") or "").strip() or (OPENAI_MODEL or MODEL or "gpt-5.2")
    claude_key = (data.get("claude_key") or "").strip()
    claude_model = (data.get("claude_model") or "").strip() or (CLAUDE_MODEL or "claude-3-5-sonnet-latest")
    ...
    append_log("user_settings_v2_updated", {"user": uname, "updated_at": now_iso(), "provider": provider})
    return jsonify({"ok": True, "settings": {
        "provider": provider,
        "openai_key_hint": _masked_key_hint(rec["settings"].get("openai_key","")),
        "claude_key_hint": _masked_key_hint(rec["settings"].get("claude_key","")),
        "openai_model": rec["settings"].get("openai_model",""),
        "claude_model": rec["settings"].get("claude_model",""),
    }})

@app.get("/api/system/health")
def api_system_health():
    return jsonify(_system_health_snapshot(run_live=False))
...
def api_system_test_text():
    ...
    try:
        if provider == "claude":
            out = _claude_text_call("Reply with OK and one short status sentence.", [{"role": "user", "content": "System health text check."}], temperature=0.1, u=u)
        else:
            out = _openai_text_call("Reply with OK and one short status sentence.", [{"role": "user", "content": "System health text check."}], temperature=0.1, u=u)
        return jsonify({"ok": True, "provider": provider, "reply": out[:300]})
    except Exception as e:
...
def api_system_test_image():
    ...
    key = _effective_openai_key(u)
    if not key:
        return jsonify({"ok": False, "error": "OpenAI key is missing"}), 400
    try:
        cli = OpenAI(api_key=key)
        resp = cli.images.generate(
            model="gpt-image-1",
            prompt="A tiny minimal blue square icon on a plain white background.",
            size="1024x1024"
        )
        b64 = ""
        mime = "image/png"
        try:
            ...
            b64 = getattr(item, "b64_json", None) or item.get("b64_json") or ""
            mime = getattr(item, "mime_type", None) or item.get("mime_type") or "image/png"
        except Exception:
            b64 = ""
        return jsonify({"ok": True, "mime_type": mime, "has_image": bool(b64)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500

@app.get("/api/system/startup_report")
def api_system_startup_report():
    return jsonify({"ok": True, "report": STARTUP_REPORT})

COMMAND_CENTER_V2_PATCH = r"""
<style>
  :root{
    --cc-panel-bg: linear-gradient(180deg, rgba(12,18,38,.96), rgba(10,14,30,.96));
    --cc-border: rgba(129,140,248,.24);
    --cc-gold: rgba(214,176,92,.45);
    --cc-shadow: 0 18px 60px rgba(0,0,0,.45), 0 0 0 1px rgba(255,255,255,.04) inset;
  }
  body.ccSafeMode *, body.ccSafeMode *::before, body.ccSafeMode *::after{
  ...
  .ccPill.ok{ background: rgba(34,197,94,.12); border-color: rgba(34,197,94,.28); }
  .ccPill.warn{ background: rgba(245,158,11,.12); border-color: rgba(245,158,11,.28); }
  .ccPill.bad{ background: rgba(239,68,68,.12); border-color: rgba(239,68,68,.28); }
  .ccBtns{ display:flex; gap:8px; flex-wrap:wrap; margin-top:10px; }
  .ccBtn{
    appearance:none; border:none; cursor:pointer;
    border-radius: 12px; padding: 9px 12px; font-size: 12px; font-weight: 700;
    color:#eef3ff; background: linear-gradient(180deg, rgba(71,85,105,.55), rgba(30,41,59,.85));
    box-shadow: 0 8px 20px rgba(0,0,0,.28), 0 0 0 1px rgba(255,255,255,.06) inset;
  }
  .ccBtn.primary{
    background: linear-gradient(180deg, rgba(79,70,229,.90), rgba(67,56,202,.92));
  }
  #ccPanel{
    position: fixed;
    right: 20px;
  ...
  #ccPanel .ccPanelBody{
    padding: 14px;
    overflow:auto;
    max-height: calc(min(78vh, 860px) - 52px);
  }
  .ccGrid{
    display:grid; grid-template-columns: 1fr 1fr; gap:12px;
  }
  .ccPanelCard{
    border-radius: 16px; padding: 12px; background: rgba(255,255,255,.04);
    border: 1px solid rgba(255,255,255,.08);
  }
  .ccPanelCard h4{ margin:0 0 8px 0; font-size:13px; }
  .ccLog{
  ...
  .ccLine{ padding: 4px 0; border-bottom:1px dashed rgba(255,255,255,.06); }
  .ccMetric{ font-size:12px; margin:4px 0; }
  .ccTag{ display:inline-block; min-width:72px; font-weight:700; opacity:.88; }
  .ccSettingsBlock{
    margin-top:12px; padding:12px; border-radius:14px;
    background: rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.08);
  }
  .ccSettingsBlock label{ display:block; font-size:12px; margin:10px 0 6px; }
  .ccSettingsBlock input, .ccSettingsBlock select{
    width:100%; border-radius: 12px; border:1px solid rgba(255,255,255,.10);
    background: rgba(8,12,24,.88); color:#eef3ff; padding: 10px 12px;
  }
  .ccStatusText{ font-size: 12px; opacity:.86; margin-top:8px; white-space: pre-wrap; }
  @media (max-width: 900px){
    #ccDock{ right: 12px; left: 12px; width:auto; }
    #ccPanel{
      right: 12px; left: 12px; bottom: 96px; width:auto; max-height: calc(100vh - 140px);
    }
...
(function(){
  const CC_LOGS = [];
  const MAX_CC_LOGS = 120;
  function $(id){ return document.getElementById(id); }
  function ts(){ try{ return new Date().toLocaleTimeString(); }catch(_){ return ""; } }
  function logLine(type, msg){
    const line = "[" + ts() + "] " + type.toUpperCase() + "  " + msg;
    CC_LOGS.unshift(line);
    if(CC_LOGS.length > MAX_CC_LOGS) CC_LOGS.pop();
    const box = $("ccLogBox");
    if(box) box.innerHTML = CC_LOGS.map(x => '<div class="ccLine">' + escapeHtml(x) + '</div>').join("");
  }
  ...
  function ensureDock(){
    ...
    dock.innerHTML = `
      <div class="ccCard">
        <div class="ccHeadRow">
          <div>
            <div class="ccTitle">Command Center Status</div>
            <div class="ccSub" id="ccSubline">Checking system…</div>
          </div>
          <div style="display:flex; gap:8px;">
            <button class="ccBtn" id="ccSafeBtn" type="button">Safe mode</button>
            <button class="ccBtn primary" id="ccOpenBtn" type="button">System</button>
          </div>
        </div>
        <div class="ccPills" id="ccPills"></div>
      </div>
    `;
    ...
    panel.id = "ccPanel";
    panel.innerHTML = `
      <div class="ccPanelBar" id="ccPanelBar">
        <div>
          <div class="ccTitle">System Health & Logs</div>
          <div class="ccSub">Built-in checks, provider tests, and client event logs</div>
        </div>
        <div style="display:flex; gap:8px;">
          <button class="ccBtn" id="ccRefreshBtn" type="button">Refresh</button>
          <button class="ccBtn" id="ccCloseBtn" type="button">Close</button>
        </div>
      </div>
      <div class="ccPanelBody">
        <div class="ccGrid">
          <div class="ccPanelCard">
            <h4>Health</h4>
            <div id="ccMetrics"></div>
            <div class="ccBtns">
              <button class="ccBtn" id="ccTestTextBtn" type="button">Test text</button>
              <button class="ccBtn" id="ccTestImageBtn" type="button">Test image</button>
            </div>
            <div class="ccStatusText" id="ccTestStatus"></div>
          </div>
          <div class="ccPanelCard">
            <h4>Live Logs</h4>
            <div class="ccLog" id="ccLogBox"></div>
          </div>
        </div>
      </div>
    `;
    ...
    $("ccOpenBtn").onclick = () => { $("ccPanel").classList.add("show"); refreshHealth(); };
    $("ccCloseBtn").onclick = () => $("ccPanel").classList.remove("show");
    $("ccRefreshBtn").onclick = () => refreshHealth();
    $("ccSafeBtn").onclick = () => {
      document.body.classList.toggle("ccSafeMode");
      logLine("mode", document.body.classList.contains("ccSafeMode") ? "Safe mode enabled" : "Safe mode disabled");
    };
    $("ccTestTextBtn").onclick = async () => {
      const st = $("ccTestStatus"); if(st) st.textContent = "Running text test…";
      try{
        const res = await fetch("/api/system/test_text", {method:"POST"});
        const data = await res.json();
        if(!data.ok) throw new Error(data.error || "Text test failed");
        if(st) st.textContent = "Text test OK via " + (data.provider||"provider") + ": " + (data.reply||"");
      }catch(e){
        if(st) st.textContent = "Text test failed: " + (e && e.message ? e.message : String(e));
      }
    };
    $("ccTestImageBtn").onclick = async () => {
      const st = $("ccTestStatus"); if(st) st.textContent = "Running image test…";
      try{
        const res = await fetch("/api/system/test_image", {method:"POST"});
        const data = await res.json();
        if(!data.ok) throw new Error(data.error || "Image test failed");
        if(st) st.textContent = "Image test OK. Returned image payload: " + (data.has_image ? "yes" : "no");
      }catch(e){
        if(st) st.textContent = "Image test failed: " + (e && e.message ? e.message : String(e));
      }
    };

    makeWindowInteractive($("ccPanel"), $("ccPanelBar"), "ccPanelBox");
  }

  function pill(label, ok, hint){
    const cls = ok === true ? "ok" : (ok === false ? "bad" : "warn");
    return '<div class="ccPill '+cls+'">' + escapeHtml(label + (hint ? ": " + hint : "")) + '</div>';
  }

  async function refreshHealth(){
    ensureDock();
    const pills = $("ccPills"), sub = $("ccSubline"), metrics = $("ccMetrics");
    try{
      const res = await fetch("/api/system/health");
      const data = await res.json();
      if(!data.ok) throw new Error(data.error || "Health check failed");
      if(sub) sub.textContent = "Provider: " + (data.provider || "openai") + " • Model: " + (data.text_model || "");
      if(pills){
        pills.innerHTML = [
          pill("Storage", !!data.storage_ok),
          pill("OpenAI", !!(data.keys && data.keys.openai && data.keys.openai.present), data.keys && data.keys.openai ? data.keys.openai.hint : ""),
          pill("Claude", !!(data.keys && data.keys.claude && data.keys.claude.present), data.keys && data.keys.claude ? data.keys.claude.hint : ""),
          pill("Images", !!(data.capabilities && data.capabilities.image)),
          pill("Speech", !!(data.capabilities && data.capabilities.speech_to_text)),
          pill("Gmail", !!(data.capabilities && data.capabilities.gmail)),
          pill("Calendar", !!(data.capabilities && data.capabilities.calendar))
        ].join("");
      }
      if(metrics){
        metrics.innerHTML = `
          <div class="ccMetric"><span class="ccTag">Provider</span>${escapeHtml(data.provider || "")}</div>
          <div class="ccMetric"><span class="ccTag">Model</span>${escapeHtml(data.text_model || "")}</div>
          <div class="ccMetric"><span class="ccTag">Storage</span>${data.storage_ok ? "Ready" : "Problem"}</div>
          <div class="ccMetric"><span class="ccTag">OAuth env</span>${data.oauth_env_ready ? "Configured" : "Missing env"}</div>
          <div class="ccMetric"><span class="ccTag">Packages</span>${escapeHtml(JSON.stringify(data.packages || {}))}</div>
        `;
      }
      logLine("health", "Refreshed system health");
    }catch(e){
      if(sub) sub.textContent = "Health check failed";
      if(metrics) metrics.innerHTML = '<div class="ccMetric">Health check failed: ' + escapeHtml(e && e.message ? e.message : String(e)) + '</div>';
      logLine("error", "Health check failed: " + (e && e.message ? e.message : String(e)));
    }
  }

  function ensureEnhancedSettingsUI(){
    const form = $("settingsForm");
    if(!form || $("ccSettingsEnhancer")) return;
    const block = document.createElement("div");
    block.id = "ccSettingsEnhancer";
    block.className = "ccSettingsBlock";
    block.innerHTML = `
      <div class="tiny" style="margin-bottom:8px;">AI routing and built-in system checks</div>

      <label>AI Provider</label>
      <select id="ccProviderSelect">
        <option value="openai">OpenAI</option>
        <option value="claude">Claude</option>
      </select>

      <label>OpenAI Model</label>
      <input id="ccOpenAIModel" placeholder="gpt-5.2" />

      <label>Claude API Key</label>
      <input id="ccClaudeKey" type="text" placeholder="sk-ant-..." autocomplete="off" autocapitalize="off" spellcheck="false" />

      <label>Claude Model</label>
      <input id="ccClaudeModel" placeholder="claude-3-5-sonnet-latest" />

      <div class="ccBtns">
        <button class="ccBtn" id="ccSettingsRefreshBtn" type="button">Refresh checks</button>
        <button class="ccBtn" id="ccSettingsTextBtn" type="button">Test text</button>
        <button class="ccBtn" id="ccSettingsImageBtn" type="button">Test image</button>
      </div>
      <div class="ccStatusText" id="ccSettingsStatusText"></div>
    `;
    const intro = form.querySelector(".tiny");
    if(intro && intro.parentNode){
      intro.parentNode.insertBefore(block, intro.nextSibling);
    }else{
      form.insertBefore(block, form.firstChild);
    }

    $("ccSettingsRefreshBtn").onclick = refreshHealth;
    $("ccSettingsTextBtn").onclick = async () => {
      const box = $("ccSettingsStatusText"); if(box) box.textContent = "Running text test…";
      try{
        const res = await fetch("/api/system/test_text", {method:"POST"});
        const data = await res.json();
        if(!data.ok) throw new Error(data.error || "Text test failed");
        if(box) box.textContent = "Text test OK via " + (data.provider||"provider") + ": " + (data.reply||"");
      }catch(e){
        if(box) box.textContent = "Text test failed: " + (e && e.message ? e.message : String(e));
      }
    };
    $("ccSettingsImageBtn").onclick = async () => {
      const box = $("ccSettingsStatusText"); if(box) box.textContent = "Running image test…";
      try{
        const res = await fetch("/api/system/test_image", {method:"POST"});
        const data = await res.json();
        if(!data.ok) throw new Error(data.error || "Image test failed");
        if(box) box.textContent = "Image test OK.";
      }catch(e){
        if(box) box.textContent = "Image test failed: " + (e && e.message ? e.message : String(e));
      }
    };
  }
  ...
  async function loadSettingsV2(){
    ...
    ensureEnhancedSettingsUI();
    try{
      const res = await fetch("/api/user/settings/v2");
      const data = await res.json();
      if(!data.ok){
        if(settingsStatus) settingsStatus.innerText = data.error || "Load failed";
        return;
      }
      const s = data.settings || {};
      const smtp = s.smtp || {};
      const openHint = s.openai_key_hint || "";
      const claudeHint = s.claude_key_hint || "";
      if($("openaiKey")){
        $("openaiKey").value = "";
        $("openaiKey").placeholder = openHint ? ("Saved (" + openHint + ") paste new to replace") : "sk-...";
      }
      if($("ccClaudeKey")){
        $("ccClaudeKey").value = "";
        $("ccClaudeKey").placeholder = claudeHint ? ("Saved (" + claudeHint + ") paste new to replace") : "sk-ant-...";
      }
      if($("ccProviderSelect")) $("ccProviderSelect").value = s.provider || "openai";
      if($("ccOpenAIModel")) $("ccOpenAIModel").value = s.openai_model || "";
      if($("ccClaudeModel")) $("ccClaudeModel").value = s.claude_model || "";
      if($("smtpHost")) $("smtpHost").value = smtp.host || "";
      if($("smtpPort")) $("smtpPort").value = smtp.port || 587;
      if($("smtpUser")) $("smtpUser").value = smtp.user || "";
      if($("smtpPass")) $("smtpPass").value = "";
      if($("smtpFromName")) $("smtpFromName").value = smtp.from_name || "";
      if(settingsStatus) settingsStatus.innerText = "Ready";
      try{ await refreshGoogleStatuses(); }catch(_){}
      try{ await refreshHealth(); }catch(_){}
    }catch(e){
      if(settingsStatus) settingsStatus.innerText = "Load failed";
    }
  }

  function installSettingsOverrides(){
    if(!$("saveSettings")) return;
    ensureEnhancedSettingsUI();
    window.loadSettings = loadSettingsV2;
    $("saveSettings").onclick = async () => {
      const st = $("settingsStatus");
      if(st) st.innerText = "Saving...";
      const payload = {
        provider: ($("ccProviderSelect") ? $("ccProviderSelect").value : "openai"),
        openai_key: ($("openaiKey") ? $("openaiKey").value : "").trim(),
        openai_model: ($("ccOpenAIModel") ? $("ccOpenAIModel").value : "").trim(),
        claude_key: ($("ccClaudeKey") ? $("ccClaudeKey").value : "").trim(),
        claude_model: ($("ccClaudeModel") ? $("ccClaudeModel").value : "").trim(),
        smtp: {
          host: ($("smtpHost") ? $("smtpHost").value : "").trim(),
          port: parseInt((($("smtpPort") ? $("smtpPort").value : "587") || "587").trim(), 10),
          user: ($("smtpUser") ? $("smtpUser").value : "").trim(),
          pass: ($("smtpPass") ? $("smtpPass").value : "").trim(),
          from_name: ($("smtpFromName") ? $("smtpFromName").value : "").trim()
        }
      };
      try{
        const res = await fetch("/api/user/settings/v2", {
        ...
        if(st) st.innerText = "Saved";
        try{ await refreshHealth(); }catch(_){}
        try{ if(typeof afterSettingsSaved === "function") await afterSettingsSaved(); }catch(_){}
      }catch(e){
        if(st) st.innerText = "Save failed: " + (e && e.message ? e.message : String(e));
      }
    };
  }

  function makeWindowInteractive(el, handle, storageKey){
    if(!el || el.dataset.ccInteractive === "1") return;
    el.dataset.ccInteractive = "1";
    if(window.innerWidth > 900){
      ...
      if(saved && typeof saved === "object"){
        ...
        if(saved.right) el.style.right = "auto";
        if(saved.bottom) el.style.bottom = "auto";
  ...
  function installInteractivity(){
    ensureDock();
    makeWindowInteractive($("modalWin"), $("modalBar"), "cc_modal_box");
    const diag = $("diagOverlay");
    const diagBar = diag ? diag.querySelector(".diagHead, .modalBar, .panelHead, .cardHead") : null;
    if(diag) makeWindowInteractive(diag, diagBar || diag, "cc_diag_box");
  }

  function installGlobalFetchLogging(){
    if(window.__ccFetchWrapped) return;
    window.__ccFetchWrapped = true;
    ...
    window.fetch = async (...args) => {
      ...
      try{
        ...
        logLine(res.ok ? "ok" : "http", (res.status || "") + " " + url);
        return res;
      }catch(e){
        logLine("fail", url + " :: " + (e && e.message ? e.message : String(e)));
        throw e;
      }
    };
  }
  ...
  window.addEventListener("error", (ev) => {
    document.body.classList.add("ccSafeMode");
    logLine("js", (ev && ev.message) ? ev.message : "Unhandled error");
  });
  window.addEventListener("unhandledrejection", (ev) => {
    document.body.classList.add("ccSafeMode");
    const reason = ev && ev.reason ? (ev.reason.message || String(ev.reason)) : "Unhandled rejection";
    logLine("promise", reason);
  });

  document.addEventListener("DOMContentLoaded", function(){
    ensureDock();
    installGlobalFetchLogging();
    installSettingsOverrides();
    installInteractivity();
    refreshHealth();
...
try:
    if "</body>" in HTML:
        HTML = HTML.replace("</body>", COMMAND_CENTER_V2_PATCH + "</body>")
    else:
        HTML += COMMAND_CENTER_V2_PATCH
except Exception:
    try:
        HTML += COMMAND_CENTER_V2_PATCH
    except Exception:
