...
<div id="operatorProfileForm" class="modalForm" style="display:none;">
  ...
  <textarea id="opm_business" class="followBox" style="min-height:90px" placeholder="What your business does..."></textarea>
  <div style="height:10px"></div>
  <div class="tiny">Offers</div>
  <textarea id="opm_offers" class="followBox" style="min-height:80px" placeholder="Your offers, pricing model, deliverables..."></textarea>
  <div style="height:10px"></div>
  <div class="tiny">Goals</div>
  <textarea id="opm_goals" class="followBox" style="min-height:70px" placeholder="Current goals and KPIs..."></textarea>
  <div style="height:10px"></div>
  <div class="tiny">Constraints</div>
  <textarea id="opm_constraints" class="followBox" style="min-height:70px" placeholder="Rules, boundaries, what not to do..."></textarea>
  <div style="height:10px"></div>
  <div class="tiny">Tone rules</div>
  <textarea id="opm_tone_rules" class="followBox" style="min-height:70px" placeholder="How teammates should speak and write..."></textarea>
  <div style="height:10px"></div>
  <div class="tiny">Notes</div>
  <textarea id="opm_notes" class="followBox" style="min-height:70px" placeholder="Anything else teammates should know..."></textarea>
  <div style="height:12px"></div>
  <div class="pillRow" style="justify-content:flex-end">
    <button class="btn" id="opmReload">Reload</button>
...
`;
    ...
    $("saveCreate").onclick = async () => {
      ...
      try{
        ...
        let data = {};
        try{ data = raw ? JSON.parse(raw) : {}; }catch(_){ throw new Error('Create teammate returned an invalid server response'); }
        if(!res.ok || !data.ok){
        ...
        if(st) st.innerText = e.message || "Create failed";
      }finally{
...
Challenge weak assumptions. Surface risks.`;
    ...
    async function saveOperatorProfileFromModal(){
      ...
      try{
        const payload = {
          display_name: $("opm_display_name")?.value || 'Operator',
...
async function crmFetchTasks(){
    ...
    if($("opmReload")) $("opmReload").onclick = ()=> loadOperatorProfileIntoModal();
    if($("opmSave")) $("opmSave").onclick = ()=> saveOperatorProfileFromModal();
